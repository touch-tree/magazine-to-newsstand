...
{    
    ...
    private function execute(&$obj)
    {
            ...
            for($n=0;$n<$len;$n++)
            {
                ...
                $char = sys::extractAlpha(strip_tags($properties['content']));
                if(sys::length($char) <> 1 )                                             { continue; }
                if(sys::strtoupper($char) !== $char)                                     { continue; }
                ...
                if(digi_pdf_to_html::nodeOverlapsBoundary($properties,$boundary) OR digi_pdf_to_html::nodeWithinBoundary($properties,$boundary) )
                {
                    ...
                    $obj['nodes'][$index2]['content'] = $char.$obj['nodes'][$index2]['content'] ;
