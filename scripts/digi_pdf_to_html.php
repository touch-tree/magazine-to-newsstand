...
class digi_pdf_to_html
{
    ...
    {	
        ...
        for($n=0; $n < $loop; $n++)
        {
            ...
            $pageWidth  =   $dom->getAttribute($pages[$n],"width"); 
            $pageHeight =   $dom->getAttribute($pages[$n],"height"); 
            
            self::$arrayPages[$pageNumber]=[];
            self::$arrayPages[$pageNumber]['meta']=     [];
            self::$arrayPages[$pageNumber]['content']=  [];
            self::$arrayPages[$pageNumber]['meta']['pageWidth']=    $pageWidth; 
            self::$arrayPages[$pageNumber]['meta']['pageHeight']=   $pageHeight; 
            ...
            for($i=0; $i < $loop2; $i++)
            {
                ...
                if( $top > $pageHeight or $left > $pageWidth  )                 {continue;}
